...
class OptimizeCAGR(AssetList):
    ...
    def maximize_return(self, target_risk: float) -> Dict[str, float]:
        """
        ...
        def objective_function(w):
            ts = Rebalance.rebalanced_portfolio_return_ts(w, ror, period=period)
            # accumulated_return = (ts + 1.).prod() - 1.
            cagr = Frame.get_cagr(ts)
            return - cagr
        ...
        risk_is_target = {'type': 'eq',
                          'fun': lambda weights: target_risk - risk_annual(weights)
                          }

        cagr_is_above = {'type': 'ineq',
                         'fun': lambda weights: objective_function(weights) - 0.05
                         }
    ...
    def ef_points(self) -> pd.DataFrame:
        ...
        target_range = self.target_risk_range
        for (i, target_risk) in enumerate(target_range):
            if i == 0:
                df = pd.DataFrame()
            start_time = time.time()
            row = self.maximize_return(target_risk)
            df = df.append(row, ignore_index=True)
            end_time = time.time()

...
- Create: `tests/portfolio/finplan/__init__.py` (empty),
  `tests/portfolio/finplan/conftest.py`,
...
Create `tests/portfolio/finplan/__init__.py` as an empty file, and
`tests/portfolio/finplan/conftest.py`:
...
`abs(hash(symbol))` is stable within a process; `PYTHONHASHSEED` randomisation
across processes does not matter because every test builds its expectations
from the same fixture instance in the same worker.

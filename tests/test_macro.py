...
def test_inflation_init_and_values_monthly(macro_patches):
    ...
    assert infl.first_date == md.m_idx[0].to_timestamp()
    assert infl.last_date == md.m_idx[-1].to_timestamp()
...
def test_set_values_monthly_happy_path(macro_patches):
    ...
    assert infl.last_date == pd.Period(next_month_str, freq="M").to_timestamp()

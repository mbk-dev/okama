...
def _init_asset(request):
    ...
    request.cls.otkr = Asset(symbol='0165-70287767.RUFUND')


@pytest.fixture(scope='class')
def _init_asset_list(request) -> None:
    request.cls.asset_list = AssetList(symbols=['RUB.FOREX', 'MCFTR.INDX'], curr='RUB',
                                       first_date='2019-01', last_date='2020-01', inflation=True)
    request.cls.currencies = AssetList(['RUBUSD.FOREX', 'EURUSD.FOREX', 'CNYUSD.FOREX'], curr='USD',
                                       first_date='2019-01', last_date='2020-01', inflation=True)
...
def _init_portfolio(request):
    request.cls.portfolio = Portfolio(symbols=['RUB.FOREX', 'MCFTR.INDX'], curr='RUB',
                                      first_date='2015-01', last_date='2020-01', inflation=True)
    request.cls.portfolio_short_history = Portfolio(symbols=['RUB.FOREX', 'MCFTR.INDX'], curr='RUB',
                                                    first_date='2019-02', last_date='2020-01', inflation=True)
    request.cls.portfolio_no_inflation = Portfolio(symbols=['RUB.FOREX', 'MCFTR.INDX'], curr='RUB',
...
def init_plots():
    return Plots(symbols=['RUB.FOREX', 'EUR.FOREX', 'MCFTR.INDX'], curr='RUB', first_date='2010-01', last_date='2020-01')
